...
class GoogleMobileAdsConsentManager private constructor(context: Context) {
  ...
  fun gatherConsent(
    ...
    gatherConsent(
      activity = activity,
      debugGeography = null,
      testDeviceHashedIds = emptyList(),
      onConsentGatheringCompleteListener = onConsentGatheringCompleteListener
    )
  }

  /**
   * Debug-only consent helper. Pass explicit test device hashed IDs and a debug geography when
   * validating UMP behavior in development builds.
   */
  fun gatherConsent(
    activity: Activity,
    debugGeography: Int?,
    ...
    if (debugGeography != null || testDeviceHashedIds.isNotEmpty()) {
      val debugSettingsBuilder = ConsentDebugSettings.Builder(activity)
      debugGeography?.let { debugSettingsBuilder.setDebugGeography(it) }
      testDeviceHashedIds
        .filter { it.isNotBlank() }
        .forEach { debugSettingsBuilder.addTestDeviceHashedId(it) }
